...
export default function RebateRecordManagement() {
  ...
  const getCashbackTier = (lossAmount: number, rebateSetup: any): string => {
    if (!rebateSetup || !rebateSetup.amountTiers) return '-';

    const applicableTier = rebateSetup.amountTiers
      .filter((tier: any) => lossAmount >= tier.validBetMoreThan)
  ...
  const getRebateRate = (lossAmount: number, rebateSetup: any): number | string => {
  ...
  const calculateCashbackAmount = (lossAmount: number, rebateRate: number | string): number => {
    if (typeof rebateRate === 'string') return 0;
    return lossAmount * (rebateRate / 100);
  };

  const getRebateMaxLimit = (rebateSetup: any): string => {
    if (!rebateSetup) return '-';
    return rebateSetup.maxRebateLimit ? `$${rebateSetup.maxRebateLimit.toFixed(2)}` : 'Unlimited';
  };
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 text-sm">
                {filteredTransactions.map((transaction) => {
                  ...
                  const cashbackTier = getCashbackTier(transaction.lossAmount || 0, rebateSetup);
                  const rebateRate = getRebateRate(transaction.lossAmount || 0, rebateSetup);
                  const cashbackAmount = calculateCashbackAmount(transaction.lossAmount || 0, rebateRate);
