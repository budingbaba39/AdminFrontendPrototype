...
// Filter transactions to only get Manual, Forfeited, and Loss Credit types that are COMPLETED
const getTransactionRecords = () => {
  return sampleOtherTransactions.filter(transaction =>
    (transaction.type === 'MANUAL' || transaction.type === 'FORFEITED' || transaction.type === 'LOSSCREDIT') &&
...
export default function TransactionRecordManagement() {
  ...
  const [transactions, setTransactions] = useState(getTransactionRecords());
  ...
  const filteredTransactions = hasSearched
    ? transactions.filter(transaction => {
        ...
        if (searchFilters.type && searchFilters.type !== 'all' && transaction.type !== searchFilters.type) return false;
    ...
    <div className="p-3 space-y-3 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900">Transaction Record Search & Filter</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          ...
          <Input
            type="date"
            placeholder="Date From (Complete Time)"
          ...
          <Input
            type="date"
            placeholder="Date To (Complete Time)"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
          ...
          <Select value={searchFilters.type} onValueChange={(value) => handleInputChange('type', value)}>
            <SelectTrigger className="h-9">
            ...
            <SelectContent>
              ...
              <SelectItem value="LOSSCREDIT">Loss Credit</SelectItem>
            </SelectContent>
          </Select>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-900 uppercase">Complete Time</th>
                    ...
                    <tr key={transaction.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">
                        <span className={`font-semibold text-xs ${
                          transaction.type === 'FORFEITED' ? 'text-red-600' : 'text-green-600'
                        }`}>
                          ${transaction.amount.toFixed(2)}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-gray-900 text-xs">{transaction.completeTime || transaction.submitTime}</td>
                      <td className="px-3 py-2 text-blue-600 text-xs">{transaction.completeBy || transaction.processBy || '-'}</td>
