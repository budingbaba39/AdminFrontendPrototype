...
export default function RebateScheduleManagement() {
  ...
  // Open edit modal
  const openEditModal = (schedule: RebateSchedule) => {
  ...
  const handleCreateSchedule = () => {
    ...
    if (formData.status === 'Active') {
      const existingActiveSchedule = schedules.find(
        s => s.rebateType === formData.rebateType && s.status === 'Active'
      );

      if (existingActiveSchedule) {
        alert(`Cannot activate schedule! A schedule with target type "${formData.rebateType}" is already active: "${existingActiveSchedule.setupName}". Please deactivate it first or set this schedule to Inactive.`);
        return;
      }
    }
    ...
    if (formData.status === 'Active') {
      const existingActiveSchedule = schedules.find(
        s => s.id !== selectedSchedule.id &&
             s.rebateType === formData.rebateType &&
             s.status === 'Active'
      );
  ...
  const formatResetFrequency = (schedule: RebateSchedule): string => {
    ...
    <div className="p-3 space-y-3 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900">Rebate Schedule Management</h2>
          <Button
        ...
        <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-[#3949ab] font-semibold text-lg">
              {showCreateModal ? 'Add Rebate Schedule' : 'Edit Rebate Schedule'}
            </DialogTitle>
          </DialogHeader>
